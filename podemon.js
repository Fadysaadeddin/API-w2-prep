...
try {
    const response = await fetch(url); // Await the fetch response
        if (response.status !== 200) { // Check for HTTP errors
...
fetchJSON(url).then((VALID_URL)=>{
    renderResults(VALID_URL)
}).catch((INVALID_URL) => {
    renderError(INVALID_URL)
})

try {
    const fetching = await fetchJSON(url);
    renderResults(fetching)
}
 catch {
